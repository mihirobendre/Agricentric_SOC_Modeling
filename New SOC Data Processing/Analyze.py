...
print("Results by Coop & Soil Type: ")
print(grouped)


###############################
# RESULTS BY SOIL TYPE ONLY ###
###############################

# Average BD using rows where BD present
bd_avg = (
    df.dropna(subset=["BD (g/cm3)"])
      .groupby(["FAO Soil Type"], as_index=False)["BD (g/cm3)"]
      .mean()
      .rename(columns={"BD (g/cm3)":"avg_BD"})
)
...
c_avg = ( 
    df.dropna(subset=["C (%)"])
      .groupby(["FAO Soil Type"], as_index=False)["C (%)"]
      .mean()
      .rename(columns={"C (%)":"avg_C"})
)

# Merge averages
grouped = pd.merge(bd_avg, c_avg, on=["FAO Soil Type"], how="outer")
...
print("Results by Soil Type only: ")
print(grouped)
